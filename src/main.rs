use std::{error::Error, fmt::format, path::PathBuf, process::exit};
...
use crate::{converters::{find_converter, get_converters, run_converter}, file_types::{FileTypeError, FileTypes}, prompts::select};
use clap::Parser;
use thiserror::Error;
...
fn get_correct_extension(file_types: &FileTypes, ext: String, prompt: String) -> Result<String, FileTypeError> {
	...
	return Ok(types[idx].clone());
}

fn get_extension(target: &PathBuf, file_types: &FileTypes) -> String {
	let prompt = format!("Multiple filetypes available. Please specify type for {:?}", &target.as_os_str());

	match target.extension()
		.and_then(|v| v.to_str())
		.map(|v| v.to_lowercase())
		.and_then(|v| Some(get_correct_extension(file_types, v, prompt)))
			{
		Some(ext) => {
			match ext {
				Ok(e) => e,
